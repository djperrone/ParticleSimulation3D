...
namespace Novaura {
	...
	std::shared_ptr<PerspectiveCamera> Application::m_Camera;
	...
	void Application::Update()
	{
		...
		if (glfwGetKey(GetWindow()->Window, GLFW_KEY_ESCAPE) == GLFW_PRESS)
			glfwSetWindowShouldClose(GetWindow()->Window, true);

		if (glfwGetKey(GetWindow()->Window, GLFW_KEY_W) == GLFW_PRESS)
			m_Camera->ProcessKeyBoard(CameraMovement::FORWARD, m_DeltaTime);

		if (glfwGetKey(GetWindow()->Window, GLFW_KEY_S) == GLFW_PRESS)
			m_Camera->ProcessKeyBoard(CameraMovement::BACKWARD, m_DeltaTime);

		if (glfwGetKey(GetWindow()->Window, GLFW_KEY_A) == GLFW_PRESS)
			m_Camera->ProcessKeyBoard(CameraMovement::LEFT, m_DeltaTime);

		if (glfwGetKey(GetWindow()->Window, GLFW_KEY_D) == GLFW_PRESS)
			m_Camera->ProcessKeyBoard(CameraMovement::RIGHT, m_DeltaTime);

		if (glfwGetKey(GetWindow()->Window, GLFW_KEY_LEFT_CONTROL) == GLFW_PRESS && !m_LeftCtrlPressed)
		{
		...
		if (glfwGetKey(GetWindow()->Window, GLFW_KEY_LEFT_CONTROL) == GLFW_RELEASE && m_LeftCtrlPressed)
		{
		...
		{
			glfwSetInputMode(GetWindow()->Window, GLFW_CURSOR, GLFW_CURSOR_DISABLED);
